...
class PackageConfig extends Config implements PackageConfigInterface
{
     ...
     * {@inheritDoc}
     */
    public function loadPackage(): static
    {
        $this->initPackageData(
            $this->getAdapter()->import($this->getFile())
        );
    ...
    protected function includeExternalConfig(): static
    {
        ...
        foreach ($includes as $filename) {
            ...
            $filename = dirname($this->getFile()) . '/' . $filename;
            ...
            $config = $this->getAdapter()->import($filename);
            
            $this->merge($config);
        }

        unset($this->conceptData['included']);
